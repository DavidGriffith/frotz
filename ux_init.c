...
*/
	...
	switch(c) {
	  ...
	  case 'Z': err_report_mode = atoi(optarg);
		    if ((err_report_mode < ERR_REPORT_NEVER) ||
			(err_report_mode > ERR_REPORT_FATAL))
		      err_report_mode = ERR_DEFAULT_REPORT_MODE;
		    break;
	}
...
void os_init_screen (void)
{
	...
	h_default_foreground =
	  (user_foreground_color == -1)
	  	? FOREGROUND_DEF : user_foreground_color;
	h_default_background =
	  (user_background_color ==-1)
		? BACKGROUND_DEF : user_background_color;
    } else
#endif
    {
...
int getconfig(char *configfile)
{
	...
	while (fgets(varname, LINELEN, fp) != NULL) {
		...
		if (strcmp(varname, "attrib_set") == 0) {
			option_attribute_assignment = getbool(value);
		}
...
explaination for this.  Because of this trouble, I've left this function
